...
List<List<int>> addNumber(List<List<int>> grid, List<List<int>> gridNew) {
  List<Point<int>> options = []; // Исправил: был Point, стал Point<int>
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (grid[i][j] == 0) {
        options.add(Point<int>(i, j)); // Явно указываем тип int
      }
    }
  }
  if (options.length > 0) {
    int spotRandomIndex = Random().nextInt(options.length);
    Point<int> spot = options[spotRandomIndex]; // Явно указываем тип
